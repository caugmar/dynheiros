...
def obter_argumentos():
    ...
    arg(parser, "-a", "--atualizar-planilhas",
        "Atualiza as planilhas de cobrança.")
    arg(parser, "-i", "--imprimir-planilhas",
        "Gera PDF das planilhas para impressão.")
    arg(parser, "-m", "--email-planilhas", "Envia planilhas por email.")
    arg(parser, "-s", "--sincronizar-planilhas",
        "Sincroniza planilhas com o servidor.")
    arg(parser, "-x", "--excluir",
        "Exclui os artefatos gerados pela cobrança mensal.")
...
def excluir_artefatos():
    ...
    files_to_delete = ["aviso.txt", "lbm.txt",
                       "minister.txt", "notas.txt", "notas.ps"]
...
def main():
    ...
    if args.gerar:
        gerar_documentos_de_cobranca()
    if args.emitir:
        emitir_documentos_de_cobranca()
    if args.relatorios:
        gerar_relatorios_de_cobranca()
    if args.recibos:
        gerar_recibos_de_aluguel()
    if args.atualizar_planilhas:
        atualizar_planilhas_dos_clientes()
