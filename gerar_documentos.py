from banco_de_dados import (proxima_nota, proximo_id_de_documento,
                            novo_documento_de_cobranca,
                            inserir_documento_de_cobranca,
                            proximo_id_de_item_de_cobranca,
                            novo_item_de_cobranca,
                            inserir_item_de_cobranca)
from carregar_dados import (carregar_dados, empresas_ativas,
                            obter_empresa_por_codigo,
...
def empresas_com_numeros(tipo):
    ...
    numeros_de_notas = list(range(proxima_nota_inicial,
                                  proxima_nota_inicial
                                  + len(lista_empresas_ativas)))
...
def inserir_documento(data_de_emissao, data_de_vencimento,
    ...
    documento = novo_documento_de_cobranca(novo_id_documento, numero_da_nota,
                                           empresa, data_de_emissao,
                                           data_de_vencimento, tipo)
    lancamentos_associados = obter_lancamentos_por_tipo_e_codigo(
        tipo, codigo_empresa)
...
def inserir_itens_de_cobranca(novo_id_documento, lancamentos_associados):
    for lancamento in lancamentos_associados:
        ...
        item = novo_item_de_cobranca(
            novo_id_item, novo_id_documento, lancamento)
...
def gerar_documentos(data_de_emissao, data_de_vencimento):
    ...
    print(f"Iniciando a geração de documentos para emissão em {
          data_de_emissao} e vencimento em {data_de_vencimento}...")
    print(f"Tipos: {tipos}.")
    for tipo in tipos:
        print(f"\nProcessando tipo: {tipo}")
        empresas_do_tipo, numeros_do_tipo = empresas_com_numeros(tipo)
        for codigo_empresa, numero_da_nota in zip(empresas_do_tipo,
                                                  numeros_do_tipo):
            empresa = obter_empresa_por_codigo(codigo_empresa)
            if empresa:  # Verifica se a empresa foi encontrada
                novo_id_doc, itens = inserir_documento(data_de_emissao,
                                                       data_de_vencimento,
                                                       tipo, codigo_empresa,
                                                       numero_da_nota, empresa)
                inserir_itens_de_cobranca(novo_id_doc, itens)
            else:
                print(f"  Aviso: Empresa com código '{codigo_empresa}'"
                      + f" não encontrada para o tipo '{tipo}'. Pulando.")
